...
class PrayerAlarmReceiver : BroadcastReceiver() {
    ...
    private fun missedChannelId(prayerId: String) = "prayer_alert_${prayerId}_missed_v1"
    ...
    private fun handleUpcoming(context: Context, intent: Intent, prayerId: String) {
        ...
        if (previousMissed) {
            ...
            // window is about to close right as this one begins. Uses a sound
            // specific to THAT prayer being missed (not this one's normal
            // sound), so e.g. "Dhuhr about to be missed" and "Asr approaching"
            // are always distinguishable even back-to-back.
            ...
            ensureChannel(context, channelId, listOf("notification_${previousId}_missed", "notification_reminder"), "Missed prayer alert for $previousId")
    ...
    private fun handleAthan(context: Context, prayerId: String) {
        ...
        if (athanMode == "full") {
            ...
            if (resId != 0) {
                try {
                    // Explicit media-stream audio attributes: this ties playback to
                    // the device's media volume, so pressing the hardware volume
                    // buttons (or muting media volume) controls/silences it live,
                    // same as any other media playback on the device.
                    val attrs = android.media.AudioAttributes.Builder()
                        .setUsage(android.media.AudioAttributes.USAGE_MEDIA)
                        .setContentType(android.media.AudioAttributes.CONTENT_TYPE_MUSIC)
                        .build()
                    val mp = MediaPlayer.create(context, android.net.Uri.parse("android.resource://${context.packageName}/$resId"), null, attrs, 0)
    ...
    private fun handleReminder(context: Context, intent: Intent, prayerId: String) {
        ...
        ensureChannel(context, channelId, listOf("notification_${prayerId}_missed", "notification_reminder"), "Missed prayer alert for $prayerId")
